...
const RippleDistortion = ({
  ...
  useEffect(() => {
    ...
    const renderer = new Renderer({
      alpha: false,
      antialias: false,
      // Required so ScrollSections' domToCanvas-based morph capture can
      // actually read this canvas's pixels — without it, a WebGL canvas
      // captures as blank, which the morph shader renders as solid black.
    ...
    // The source image loads asynchronously (see image.onload below), so the
    // canvas paints solid black for a bit before the texture is ready. Flag
    // that so ScrollSections' waitForCanvasesReady() (useSectionTextures.js)
    // knows not to snapshot this canvas for the morph transition until the
    // image has actually loaded — without it, the section's morph texture
    // would get captured mid-black-frame and stay that way forever, since
    // captures are cached.
    canvas.dataset.asyncReady = 'false';
    ...
    image.onload = () => {
      ...
      // Wait one more frame so the texture has actually been uploaded and
      // drawn at least once before flagging ready — see the comment above
      // canvas.dataset.asyncReady's initial assignment.
      requestAnimationFrame(() => {
        if (!disposed) canvas.dataset.asyncReady = 'true';
      });
    };
    image.src = src;
